...
while enter_player.upper() !='X':
    ...
    if os.path.isfile('players.txt'):
        enter_player = input("Would you like to create a team or manage an existing team?\n (Enter 'C' for create, 'M' for manage, 'X' to exit) ")
    else:
    ...
    #Enter a player for the team
        print('Enter a list of players n our team along with their position')
        enter_cont ='Y'
        ...
        while enter_cont.upper()=='Y':
            ...
            #Assign position to a dictionary key bof the player name 
            player_dict[name] = position
            enter_cont = input("Enter another player? (Press 'N' to exit or 'Y' to continue)")
        else:
            enter_player = 'X'
    ...
    #Manage player.txt entries
        ...
        #Use the for-loop to interate over the entries in the file
        ...
        for i, player in enumerate(player_dict):
            print('Player % Name: %s -- Position: %s' %(i, player, player_dict[player]))
